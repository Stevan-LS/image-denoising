...
parser.add_argument('--data_dir', type=str,
                    default='./data/train/OpenImages_train')
    ...
    def add_train_noise(self, x):
        ...
        if self.style == "gauss_fix":
            ...
            torch.normal(mean=0.0,
                        std=std,
                        generator=get_generator(x.device),
                        out=noise)
            return x + noise
            ...
            noise = torch.cuda.FloatTensor(shape, device=x.device)
            torch.normal(mean=0, std=std, generator=get_generator(x.device), out=noise)
            return x + noise
        elif self.style == "poisson_fix":
            lam = self.params[0]
            lam = lam * torch.ones((shape[0], 1, 1, 1), device=x.device)
            noised = torch.poisson(lam * x, generator=get_generator(x.device)) / lam
            return noised
            ...
            # Traitement pour le bruit de forge avec PyTorch
            device = x.device
            batch_noisy = torch.zeros_like(x)
            
            for i in range(shape[0]):
                # Convertir en CPU et à NumPy pour le traitement
                img_cpu = x[i].cpu().numpy().transpose(1, 2, 0)
                
                # Convertir à [0, 255] pour le traitement
                img_255 = (img_cpu * 255).astype(np.uint8)
                
                # Appliquer le bruit de forge
                noisy_img = self._add_forge_noise_numpy(img_255, 
                                                    self.poussiere_intensity,
                                                    self.calamine_spots,
                                                    self.grain_strength)
                
                # Normaliser entre [0,1] et remettre au format channel-first
                noisy_norm = noisy_img.astype(np.float32) / 255.0
                noisy_tensor = torch.from_numpy(noisy_norm).permute(2, 0, 1).to(device)
                
                # Ajouter au batch
                batch_noisy[i] = noisy_tensor
...
def validation_bsd500_val(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "*"))
    fns.sort()
    images = []
    for fn in fns:
...
def validation_OpenImages(dataset_dir):
    fns = glob.glob(os.path.join(dataset_dir, "*"))
    fns.sort()
    images = []
    for fn in fns:
...
BSD500_val_dir = os.path.join(opt.val_dirs, "BSD_500_val")
OpenImages_dir = os.path.join(opt.val_dirs, "OpenImages_val")
...
if opt.checkpoint is not None and opt.resume is None:
    # Si on a juste un checkpoint sans fichier d'état, commencer à l'époque suivante
    # en supposant que le checkpoint est nommé "epoch_model_XXX.pth"
    checkpoint_name = os.path.basename(opt.checkpoint)
    epoch_match = re.search(r'epoch_model_(\d+)\.pth', checkpoint_name)
    if epoch_match:
        epoch_init = int(epoch_match.group(1)) + 1
    else:
        # Fallback si le nom ne correspond pas au format attendu
        epoch_init = 13  # Spécifiez l'époque manuellement ici
    
    # Mettre à jour le scheduler jusqu'à l'époque initiale
    for i in range(1, epoch_init):
        scheduler.step()
    
    new_lr = scheduler.get_last_lr()[0]  # get_last_lr() au lieu de get_lr()
    logger.info('----------------------------------------------------')
    logger.info("==> Resuming Training from epoch {} with learning rate:{}".format(epoch_init, new_lr))
    logger.info('----------------------------------------------------')
...
for epoch in range(epoch_init, opt.n_epoch + 1):
    ...
    if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
        ...
        np.random.seed(101)
        valid_repeat_times = {"Kodak24": 10, "BSD300": 3, "Set14": 20, "BSD500_val": 3, "OpenImages_val":1}
